...
public class UserService {
	...
	// @HystrixCommand(fallbackMethod = "fallback_authenticate")
	public boolean authenticateForPassApp(final String username, final String password) {
		String filter = "(&(|(uid=" + username + ")(mail=" + username + ")(mailequivalentaddress=" + username + ")))";
		log.debug("Filter for authentication : {} ", filter);
		return ldapTemplate.authenticate(ldapProperties.getBaseDN(), filter, password);
	}
...
//		return dnBuilder.add("ou", "People").add("uid", user.getUsername()).build();
	...
	public List<String> autoSuggestion(@NotBlank String mail) {
		mail = "*"+mail+"*";
		List<UserForSearch> user = ldapTemplate.find(query().base(ldapProperties.getBaseDN()).where("uid").like(mail)
				.or("mail").like(mail).or("mailequivalentaddress").like(mail), UserForSearch.class);
		List<String> emails = new ArrayList<>();
		if (user.size() == 0) {
			log.debug("mail : {} could not be found", mail);
			return null;
		} else {
			for (UserForSearch userValues : user) {
				emails.add(userValues.getEmail());
			}
		}
		return emails;
	}
